...
dpd.lits = dpd("/lits");
dpd.lits.on = function(ev, fn) {
  return dpd.on("lits" + ":" + ev, fn);
}
dpd.lits.once = function(ev, fn) {
  return dpd.once("lits" + ":" + ev, fn);
}
dpd.lits.off = function(ev, fn) {
  return dpd.off("lits" + ":" + ev, fn);
}
dpd.lits.rename = function (path, body, fn) {
  dpd.lits.exec("rename", path, body, fn);
}
